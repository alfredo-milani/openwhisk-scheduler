...
import it.uniroma2.faas.openwhisk.scheduler.scheduler.advanced.HealthCheckerScheduler;
...
public class HealthCheckerSchedulerMock extends AdvancedScheduler {
    ...
    private final SchedulerExecutors executorService = new SchedulerExecutors(DEFAULT_THREAD_COUNT, 0);
    //
    ...
    public HealthCheckerSchedulerMock(@Nonnull Scheduler scheduler) {
        super(scheduler);
    }
        ...
        if (stream.equals(HEALTH_STREAM)) {
                    ...
                    if (!invokerState.getKey().equals(State.HEALTHY)) {
                        ...
                        if (invokerQueue != null) {
                            ...
                            if (invokerQueue.size() > 0) {
                                super.newEvent(ActivationKafkaConsumer.ACTIVATION_STREAM, List.of(invokerQueue));
            ...
            for (IConsumable consumable : consumables) {
                if (consumable instanceof ISchedulable) {
                    final ISchedulable schedulable = (ISchedulable) consumable;
